...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoggedIn, backendUrl = 'http://localhost:4000' }) => {
  ...
  const handleForgot = async () => {
    ...
    if (!r.ok) {
      ...
      const data = await r.json();
      addToast('Recovery email sent. Check your inbox.', 'success');
    }
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.98 }} className="relative w-full max-w-md bg-[#0c0c0e] border border-white/10 rounded-2xl shadow-premium overflow-hidden">
            ...
            <div className="px-5 pb-5 space-y-4">
              {mode === 'register' && (
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input value={username} onChange={(e) => setUsername(e.target.value)} className="w-full bg-[#18181b] border border-white/10 rounded-lg py-2.5 pl-9 pr-4 text-sm text-white focus:border-white/30 outline-none transition-all" placeholder="Your display name" />
                  </div>
                </div>
              )}
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-[#18181b] border border-white/10 rounded-lg py-2.5 pl-9 pr-4 text-sm text-white focus:border-white/30 outline-none transition-all" placeholder="you@example.com" />
                </div>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleSubmit} disabled={isSubmitting} className="flex-1 px-4 py-2.5 bg-white text-black text-sm font-semibold rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50">
                  {mode === 'login' ? <span className="inline-flex items-center gap-1"><LogIn size={14} /> Sign In</span> : <span className="inline-flex items-center gap-1"><UserPlus size={14} /> Create</span>}
                </button>
            ...
            <div className="pt-3">
              <div className="text-[11px] text-gray-500 mb-2">Or continue with</div>
            ...
            <div className="px-5 pb-5">
              <p className="text-[11px] text-gray-500 text-center">
                By signing in you agree to our <a href="/terms.html" className="underline hover:text-white">Terms of Service</a> and <a href="/privacy.html" className="underline hover:text-white">Privacy Policy</a>.
              </p>
            </div>
            </div>
          </motion.div>

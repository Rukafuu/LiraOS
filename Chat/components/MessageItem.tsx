...
export const MessageItem: React.FC<MessageItemProps> = ({
  ...
  // ... (handleCopyCode logic preserved if needed, but not in original context to retain)
            ...
            {message.attachments && message.attachments.length > 0 && (
              <div className={`flex flex-wrap gap-2 mb-2 ${isUser ? 'justify-end' : 'justify-start'}`}>
                {message.attachments.map((att, idx) => (
                  <div key={idx} className="relative group overflow-hidden rounded-lg border border-white/10 bg-black/40 cursor-pointer hover:border-purple-500/50 transition-colors">
                    {att.type === 'image' ? (
                      <img
                        src={att.previewUrl}
                        alt="attachment"
